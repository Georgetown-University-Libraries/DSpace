...
public class SiteRestRepository extends DSpaceRestRepository<SiteRest, UUID> {
	SiteService cs = ContentServiceFactory.getInstance().getSiteService();
	@Autowired
	...
	public Page<SiteRest> findAll(Context context, Pageable pageable) {
		...
		try {
			sites.add(cs.findSite(context));
		} catch (SQLException e) {
			throw new RuntimeException(e.getMessage(), e);
		}
		Page<SiteRest> page = new PageImpl<Site>(sites, pageable, sites.size()).map(converter);
		return page;
	}
	...
	@Override
	public SiteResource wrapResource(SiteRest community, String... rels) {
		return new SiteResource(community, utils, rels);
	}
